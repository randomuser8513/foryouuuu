<template>
  ...
  <dialog ref="dialogRef" class="flower-dialog" @cancel.prevent>
    ...
    <template v-if="dialogStep === 'ask'">
      ...
      <div class="dialog-buttons">
        <button class="fall-button" @click="answerYes">Sí</button>
        <button class="fall-button" @click="answerNo">No</button>
      </div>
    </template>

    <template v-if="dialogStep === 'sure'">
      <div style="color: white">Estás segura?</div>
      <div class="dialog-buttons">
        <button ref="runawayRef" class="fall-button runaway" :style="{ transform: `translate(${runawayOffset.x}px, ${runawayOffset.y}px)` }" @mouseover="moveRunaway" @click="sureYes">Sí</button>
...
const specialCell = ref(0)
const partyCell = ref(0)
const shuffled = ref<string[]>([])

function updateRandomLayout() {
  const total = totalCells.value
  if (total <= 0) return

  specialCell.value = Math.floor(Math.random() * total) + 1
...
watch(totalCells, updateRandomLayout, { immediate: true })
